...
export class WebhooksController {
  ...
  private async processPostback(
    ...
    if (payload['pts'] !== undefined && payload['pts'] !== '') {
      ...
      rewardPoints = parseInt(String(payload['credit']), 10);
    ...
    if (rewardPoints <= 0) {
      const rawPayout = String(payload['payout'] ?? payload['points'] ?? payload['user_amount'] ?? payload['amount'] ?? '');
      if (rawPayout.startsWith('{') || rawPayout.toLowerCase() === 'test') {
    ...
    const isAutoGeneratedTx = !rawTxId || rawTxId.startsWith('{');
    ...
    if (!user && (!userId || userId.startsWith('{') || userId.toLowerCase() === 'test')) {
      user =
